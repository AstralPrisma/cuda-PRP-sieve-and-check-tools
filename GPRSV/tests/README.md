...
per group (Linux, from `GPRSV/`, target adjusted to the actual GPU):
...
for compilation. The main prebuilt GPRSV program does not require nvcc to run.
...
`--version` in GPRSV includes its banner and the final version line. Normal
