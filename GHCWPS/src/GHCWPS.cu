...
#include <regex>
#include <sstream>
#ifndef GHCWPS_FUSED_RELAX
#define GHCWPS_FUSED_RELAX 1
#endif
#if GHCWPS_FUSED_RELAX != 0 && GHCWPS_FUSED_RELAX != 1
#error GHCWPS_FUSED_RELAX must be 0 or 1
#endif
#ifdef _WIN32
#ifndef NOMINMAX
#define NOMINMAX
...
public:
    Montgomery(const cpp_int& modulus,uint32_t witness,bool use_graphs,int block_cap)
        ...
        std::cout<<"montgomery: radix_bits="<<BITS<<", limbs="<<m_<<", ntt_length="<<len_
                 ...
                 <<", carry_normalize="<<(GHCWPS_FUSED_RELAX?"fused3+scan":"baseline3+scan")<<"\n";
    }
...
using Expression=ghcw_model::Expression;
Expression parse(const std::string& text) { return ghcw_model::parse(text); }
cpp_int modulus(const Expression& e) {
    return ghcw_model::modulus(e,[]{return stop_requested.load(std::memory_order_relaxed);});
}
...
std::vector<uint8_t> checkpoint_data(const Expression& e,uint32_t witness,const cpp_int& n,
    ...
    std::vector<uint8_t> out={'G','H','C','W','P','S','0','1'};
    for(uint64_t x:{uint64_t(1),uint64_t(3),e.b,e.n,uint64_t(e.c+1),uint64_t(witness),uint64_t(v.size()),
...
std::pair<uint64_t,std::vector<uint32_t>> read_checkpoint(const std::filesystem::path& path,
    ...
    if(std::memcmp(bytes.data(),"GHCWPS01",8)!=0) throw std::runtime_error("not a GHCWPS checkpoint");
    size_t pos=8;
    auto take64=[&](){ uint64_t x=0; for(int j=0;j<8;++j) x|=uint64_t(bytes[pos++])<<(8*j); return x; };
    for(uint64_t want:{uint64_t(1),uint64_t(3),e.b,e.n,uint64_t(e.c+1),uint64_t(witness),uint64_t(m),
...
int run(int argc,char** argv) {
    if(argc==2 && std::string(argv[1])=="--version"){std::cout<<"GHCWPS "<<kVersion<<"\n";return 0;}
    std::string expression,checkpoint; uint32_t witness=2; uint64_t max_bits=0,every=100000,progress=100000;
    bool resume=false,verify=false,graphs=true,print_residue=false,help=false,interval_given=false,cpu_reference=false;
    ...
    for(int i=1;i<argc;++i) {
        ...
        else if(arg=="--cpu-reference") cpu_reference=true;
    ...
    if(help || expression.empty()) {
        ::display_banner();
        std::cout << "GHCWPS --check \"b^n*n^b+/-1\" [options]\n"
                  << "2<=b,n<=4294967295; decimal integers; quote the expression.\n"
                  << "--witness 2..255 --max-bits N --checkpoint FILE\n"
                  << "--checkpoint-every-bits N --resume-checkpoint\n"
                  << "--progress-every-bits N (default100000) --force-ntt-blocks N\n"
                  << "--no-graphs --verify-cpp-int --print-residue\n"
                  << "--cpu-reference (test-only, <=8192 bits, no CUDA calls)\n"
                  << "Fermat PRP only, not a primality proof.\n";
        return 0;
    }
    ...
    std::cout<<"GHCWPS "<<kVersion<<": N="<<e.text()<<", witness="<<witness<<"; constructing modulus\n";
    auto algebraic=ghcw_model::composite_reason(e);
    if(!algebraic.empty() && !resume && !cpu_reference) {
        std::cout<<"ghcwps-check: N="<<e.text()<<", result=COMPOSITE, reason="<<algebraic<<"\n";return 0;
    }
    cpp_int n=ghcwps::modulus(e), exponent=n-1;
    ...
    if(n==2) { std::cout<<"ghcwps-check: N="<<e.text()<<", result=PRP, reason=trivial-prime-2\n"; return 0; }
    if((n&1)==0) { std::cout<<"ghcwps-check: N="<<e.text()<<", result=COMPOSITE, factor=2\n"; return 0; }
    ...
    if(g>1) { if(n==g) throw std::runtime_error("invalid witness: divisible by candidate");
        std::cout<<"ghcwps-check: N="<<e.text()<<", result=COMPOSITE, factor="<<g<<"\n"; return 0; }
    ...
    if(cpu_reference) {
        if(nbits>8192) throw std::runtime_error("CPU reference capped at8192bits");
        const unsigned needed=boost::multiprecision::msb(n*witness)+1;
        const int m=int((needed+BITS-1)/BITS);const cpp_int r=cpp_int(1)<<(BITS*m);
        cpp_int value=1;
        // R inverse modulo an odd, possibly composite N: extended Euclid.
        if(resume) {
            cpp_int a=r,b=n,x=1,y=0;
            while(b!=0) {cpp_int q=a/b,rem=a-q*b,t=x-q*y;a=b;b=rem;x=y;y=t;}
            if(a!=1)throw std::runtime_error("invalid radix inverse");
            value=(from_limbs(saved_digits)*((x%n+n)%n))%n;
        }
        uint64_t limit=max_bits?std::min(bits,done+std::min(max_bits,bits-done)):bits;
        while(done<limit && !stop_requested.load()) {
            value=(value*value)%n;
            if(boost::multiprecision::bit_test(exponent,unsigned(bits-1-done)))value=value*witness%n;
            ++done;
        }
        if(!checkpoint.empty())atomic_write(path,checkpoint_data(e,witness,n,bits,done,limbs(value*r%n,m)));
        std::cout<<"ghcwps-check: N="<<e.text()<<", processed_bits="<<done<<"/"<<bits
                 <<", backend=cpu-reference, result="<<(done<bits?"PARTIAL":value==1?"PRP":"COMPOSITE")<<"\n";
        if(print_residue)std::cout<<"residue="<<value<<"\n";
    ...
    std::cout<<"ghcwps-check: N="<<e.text()<<", witness="<<witness<<", processed_bits="<<done<<"/"<<bits
...
void display_banner() {
    ...
    printf("%s\n","       .oooooo.        ooooo   ooooo       .oooooo.      oooooo   oooooo     oooo    ooooooooo.       .oooooo..o         ");
    printf("%s\n","      d8P'  `Y8b       `888'   `888'      d8P'  `Y8b      `888.    `888.     .8'     `888   `Y88.    d8P'    `Y8         ");
    printf("%s\n","     888                888     888      888               `888.   .8888.   .8'       888   .d88'    Y88bo.              ");
    printf("%s\n","     888                888ooooo888      888                `888  .8'`888. .8'        888ooo88P'      `'Y8888o.          ");
    printf("%s\n","     888     ooooo      888     888      888                 `888.8'  `888.8'         888                 `'Y88b         ");
    printf("%s\n","     `88.    .88'  .o.  888     888  .o. `88b    ooo  .o.     `888'    `888'     .o.  888        .o. oo     .d8P .o.     ");
    printf("%s\n","      `Y8bood8P'   Y8P o888o   o888o Y8P  `Y8bood8P'  Y8P      `8'      `8'      Y8P o888o       Y8P 8''88888P'  Y8P     ");
    printf("%s\n","");
    printf("%s\n","                                     Generalized Hyper Cullen / Woodall Prime Seeker                                     ");
...
int main(int argc,char** argv) {
    ...
    std::signal(SIGINT,ghcwps::signal_handler); std::signal(SIGTERM,ghcwps::signal_handler);
#ifdef _WIN32
    SetConsoleCtrlHandler(ghcwps::console_handler,TRUE);
#endif
    try {
#ifdef _WIN32
        ...
        return ghcwps::run(count,args.data());
#else
        return ghcwps::run(argc,argv);
#endif
    }
    catch(const std::exception& e) { std::cerr<<"error: "<<e.what()<<"\n"; return ghcwps::stop_requested.load()?130:1; }
}
