// Intentionally included twice with distinct ghcwps namespaces.
...
#ifndef GHCWPS_RADIX_BITS
#define GHCWPS_RADIX_BITS 15
#endif
static_assert(GHCWPS_RADIX_BITS==15||GHCWPS_RADIX_BITS==18||GHCWPS_RADIX_BITS==21,"supported compiled radix");
constexpr uint32_t BITS=GHCWPS_RADIX_BITS, BASE=1u<<BITS, MASK=BASE-1;
constexpr const char* CHECKPOINT_MAGIC=BITS==15?"GHCWPS01":BITS==18?"GHCW18A1":"GHCW21A1";
...
std::vector<uint8_t> checkpoint_data(const Expression& e,uint32_t witness,const cpp_int& n,
    ...
    std::vector<uint8_t> out(CHECKPOINT_MAGIC,CHECKPOINT_MAGIC+8);
...
std::pair<uint64_t,std::vector<uint32_t>> read_checkpoint(const std::filesystem::path& path,
    ...
    if(std::memcmp(bytes.data(),CHECKPOINT_MAGIC,8)!=0) throw std::runtime_error("checkpoint radix/format mismatch");
    size_t pos=8;
...
int run(int argc,char** argv) {
    ...
    if(help || expression.empty()) {
        ...
        std::cout << "GHCWPS --check \"b^n*n^b+/-1\" [options]\n"
                  ...
                  << "--radix-bits auto|15|18|21 (default auto; checkpoint format preserved)\n"
