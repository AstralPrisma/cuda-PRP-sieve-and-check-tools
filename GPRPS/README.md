...
GPRPS is a CUDA Fermat probable-prime checker for generalized Proth/Riesel candidates

```text
N = k*b^n + c,  where c is +1 or -1
```

Unlike GFPS, the GPRPS command-line argument `n` is the exponent itself.

> 中文提示：GPRPS 的 `n` 就是表达式中的指数。输出 `PRP` 只代表通过指定底数的 Fermat 测试，不是确定性素数证明。

## Result status

For a selected Fermat witness `a` (2 by default), GPRPS evaluates
...
Optional diagnostic builds may define `GPRPS_GMP_DIAGNOSTIC` and link GMP/GMPXX. Normal builds do not require GMP.

## Build

From the `GPRPS` directory on Linux or WSL:
...
```bash
./GPRPS --check '326*799^325799-1' 3
./GPRPS --check 326 799 325799 -1 3
```
...
```text
gprps-check: N=326*799^325799-1, witness=2, ..., raw_one=no,
             checksum=..., verification=not-requested, result=COMPOSITE
```

Automation should parse the final `gprps-check:` line and its `result=PRP` or `result=COMPOSITE` field. A progress line reaching 100% is not by itself a result receipt.
...
--tuning-cache-dir <path>     Persistent tuning cache; default .gprps_tuning_cache.
...
```text
GPRPS_NTT_BLOCKS=1..4096
GPRPS_WINDOW_BITS=1..8
GPRPS_DUTY_PERCENT=1..100
GPRPS_TUNING_CACHE_DIR=<path>
GPRPS_TUNING_CACHE_MAX_AGE_HOURS=1..8760
...
Version 2.4 uses weighted fixed-divisor scans and compact carry reduction when
...
```bash
./GPRPS --bench-square <k> <b> <n> <c:+1|-1> <iterations>
./GPRPS --bench-mul    <k> <b> <n> <c:+1|-1> <iterations>
```
...
```bash
./GPRPS --checkpoint-info work.ckpt
```
...
The `.gprps_tuning_cache` directory remains separate: it only caches performance
...
Run `./GPRPS` without arguments to print the authoritative usage summary for the exact binary being used.
